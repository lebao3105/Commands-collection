...
Some CC programs do work the same way as their original counterpart: hello, uptime and uname are 2 examples.
...
* Planned: macOS, need maintainers (hackintosh is still dying)
* WIP: Windows, need maintainers (I do still have Windows installed, but spend little to no time with it)
...
Q: Hmm, CC looks like a C/C++ project...Can you make it more familiar to us:D?

A: Yes and no.

0. Some type aliases are already made, for example int(eger) and bool(ean).
1. Creating a main function like what we do in C/C++:

```pascal
function main(argc: int; argv: array of pchar): int;
```

And tell FPC to use that using `-XM<name>` flag or `{$PASCALMAINNAME}`.

2. Limit the use of RTL and FCL. You'll need to reinvent a lot of wheels.
3. Remove use of Pascal features and replace them with things found in C. For example PChar instead of String, remove `initialization` and `finalization` blocks

## Versioning system

CC follows this YYDDMM\<branch\> for versions, where \<branch\> is one of:
