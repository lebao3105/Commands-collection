...
override COMPILER_OPTIONS+=-gl -Sa -Si -Sm -Sc -Sx -Co -CO -Cr -CR -dbg:=begin -ded:=end -dretn:=procedure -dfn:=function -dlong:=longint -dulong:=longword -dint:=integer -dbool:=boolean
endif
...
override REQUIRE_PACKAGES=regexpr fcl-process rtl
ifeq ($(FULL_TARGET),i386-linux)
