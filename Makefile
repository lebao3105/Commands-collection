...
override COMPILER_OPTIONS+=-gl -Sa -Si -Sm -Sc -Sx -Co -CO -Cr -CR -dbg:=begin -ded:=end -dretn:=procedure -dfn:=function -dlong:=longint -dulong:=longword -dint:=integer -dbool:=boolean -dREV:=\'$(GIT) rev-parse --short HEAD\'
endif
...
.PHONY: all debug smart release units shared install sourceinstall exampleinstall distinstall zipinstall zipsourceinstall zipexampleinstall zipdistinstall clean distclean cleanall info makefiles
