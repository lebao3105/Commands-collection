...
override PACKAGE_NAME=Commands Collection
override PACKAGE_VERSION=1.1.0
ifeq ($(CPU_OS_TARGET),i386-linux)
override COMPILER_OPTIONS+=-gl -Sa -Si -Sm -Sc -Sx -Co -CO -Cr -CR -dbg:=begin -ded:=end -dretn:=procedure -dfn:=function -dlong:=longint -dulong:=longword -dint:=integer -dbool:=boolean -dreturn:=exit
endif
...
ifeq ($(CPU_OS_TARGET),i386-linux)
override COMPILER_INCLUDEDIR+=include src
endif
ifeq ($(CPU_OS_TARGET),i386-go32v2)
override COMPILER_INCLUDEDIR+=include src
endif
...
ifeq ($(CPU_OS_TARGET),i386-linux)
override COMPILER_UNITDIR+=include src
endif
ifeq ($(CPU_OS_TARGET),i386-go32v2)
override COMPILER_UNITDIR+=include src
endif
...
ifeq ($(CPU_OS_TARGET),i386-linux)
override COMPILER_SOURCEDIR+=include src
endif
ifeq ($(CPU_OS_TARGET),i386-go32v2)
override COMPILER_SOURCEDIR+=include src
endif
...
ifeq ($(CPU_OS_TARGET),i386-linux)
override COMPILER_TARGETDIR+=build/progs
endif
ifeq ($(CPU_OS_TARGET),i386-go32v2)
override COMPILER_TARGETDIR+=build/progs
endif
...
ifeq ($(CPU_OS_TARGET),i386-linux)
override COMPILER_UNITTARGETDIR+=build/obj_out
endif
ifeq ($(CPU_OS_TARGET),i386-go32v2)
override COMPILER_UNITTARGETDIR+=build/obj_out
endif
...
override REQUIRE_PACKAGES=regexpr fcl-process users rtl
ifeq ($(CPU_OS_TARGET),i386-linux)
...
ifeq ($(PACKAGEDIR_REGEXPR),)
PACKAGEDIR_REGEXPR:=$(firstword $(subst /Makefile,,$(strip $(wildcard $(addsuffix /regexpr/Makefile,$(PACKAGESDIR))))))
ifneq ($(PACKAGEDIR_REGEXPR),)
PACKAGEDIR_REGEXPR:=$(firstword $(subst /fpmake.pp,,$(strip $(wildcard $(addsuffix /regexpr/fpmake.pp,$(PACKAGESDIR))))))
endif
endif
ifneq ($(PACKAGEDIR_REGEXPR),)
...
ifdef REQUIRE_PACKAGES_USERS
PACKAGEDIR_USERS:=$(firstword $(subst /Makefile.fpc,,$(strip $(wildcard $(addsuffix /users/Makefile.fpc,$(PACKAGESDIR))))))
ifeq ($(PACKAGEDIR_USERS),)
PACKAGEDIR_USERS:=$(firstword $(subst /Makefile,,$(strip $(wildcard $(addsuffix /users/Makefile,$(PACKAGESDIR))))))
ifneq ($(PACKAGEDIR_USERS),)
PACKAGEDIR_USERS:=$(firstword $(subst /fpmake.pp,,$(strip $(wildcard $(addsuffix /users/fpmake.pp,$(PACKAGESDIR))))))
endif
endif
ifneq ($(PACKAGEDIR_USERS),)
ifneq ($(wildcard $(PACKAGEDIR_USERS)/units/$(TARGETSUFFIX)),)
UNITDIR_USERS=$(PACKAGEDIR_USERS)/units/$(TARGETSUFFIX)
else
UNITDIR_USERS=$(PACKAGEDIR_USERS)
endif
ifneq ($(wildcard $(PACKAGEDIR_USERS)/units/$(SOURCESUFFIX)),)
UNITDIR_FPMAKE_USERS=$(PACKAGEDIR_USERS)/units/$(SOURCESUFFIX)
else
ifneq ($(wildcard $(PACKAGEDIR_USERS)/units_bs/$(SOURCESUFFIX)),)
UNITDIR_FPMAKE_USERS=$(PACKAGEDIR_USERS)/units_bs/$(SOURCESUFFIX)
else
UNITDIR_FPMAKE_USERS=$(PACKAGEDIR_USERS)
endif
endif
ifdef CHECKDEPEND
$(PACKAGEDIR_USERS)/$(FPCMADE):
	$(MAKE) -C $(PACKAGEDIR_USERS) $(FPCMADE)
override ALLDEPENDENCIES+=$(PACKAGEDIR_USERS)/$(FPCMADE)
endif
else
PACKAGEDIR_USERS=
UNITDIR_USERS:=$(subst /Package.fpc,,$(strip $(wildcard $(addsuffix /users/Package.fpc,$(UNITSDIR)))))
ifneq ($(UNITDIR_USERS),)
UNITDIR_USERS:=$(firstword $(UNITDIR_USERS))
else
UNITDIR_USERS=
endif
endif
ifdef UNITDIR_USERS
override COMPILER_UNITDIR+=$(UNITDIR_USERS)
endif
ifdef UNITDIR_FPMAKE_USERS
override COMPILER_FPMAKE_UNITDIR+=$(UNITDIR_FPMAKE_USERS)
endif
endif
...
ifndef GIT
GIT:=$(strip $(wildcard $(addsuffix /git$(SRCEXEEXT),$(SEARCHPATH))))
ifeq ($(GIT),)
GIT= __missing_command_GIT
else
GIT:=$(firstword $(GIT))
endif
endif
...
.PHONY: debug smart release units shared install sourceinstall exampleinstall distinstall zipinstall zipsourceinstall zipexampleinstall zipdistinstall clean distclean cleanall info makefiles
...
%: src/%.pp
	$(COMPILER) $<
	$(EXECPPAS)
all: calltime cat chk_type dir env inp mkdir rename rm rmdir touch uname uptime
examples:
	@echo There are no examples here. Exiting.
	@exit 1
